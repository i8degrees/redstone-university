...
&#xNAN;_&#x46;igure: A simple, controllable Redstone clock. The repeater loop on the left creates a pulse, which is controlled by the RUN/HALT lever via the AND gate on the right._\
...
1. **The Program Counter (PC):** A special register that holds the memory address of the _next instruction_ to be executed. On every clock tick, it usually just increments by one.
2. **The Instruction Register (IR):** A register that holds the _current instruction_ after it's been fetched from RAM. This is important because it holds the instruction steady while the PC is already getting ready for the next one.
...
_(Note: We will need to add two simple 4-bit registers, A and B, to hold the inputs for the ALU. These are just like the register we built in Module 10.)_
...
1. **The Build:** This is an integration challenge.
   ...
   * Carefully connect all the components: the PC's output goes to the RAM's address input. The RAM's output goes to the Instruction Register. The IR's opcode part goes to the decoder. The decoder's control lines go to _everything_ (the ALU's opcode, the registers' write enable lines, etc.).
...
&#xNAN;_&#x46;igure: The final, assembled 4-bit computer. All components, the ALU, RAM, and Control Unit, are connected and ready to execute a program._\
...
1\. \*\*Fetch:\*\* Get the instruction from memory. \*\*Decode:\*\* Determine what the instruction means. \*\*Execute:\*\* Activate the correct components to perform the instruction. 2. The \*\*Program Counter (PC)\*\* holds the address of the \*next\* instruction to be fetched. The \*\*Instruction Register (IR)\*\* holds the \*current\* instruction that is being decoded and executed. 3. The \*\*Status Flags\*\* (specifically, the Zero Flag in this case). The Control Unit's decision to jump is based on the state of this flag.
...
\`\`\`assembly LDI A, 5 // Load the number 5 into Register A LDI B, 3 // Load the number 3 into Register B SUB // Subtract B from A STA // Store the result in RAM address 10 (0xA) HLT // Halt \`\`\`

...
4. The **NOT** gate (a Redstone Torch) and the **OR** gate (merging Redstone Dust lines).
5. A truth table's purpose is to define a gate's behavior for every possible combination of inputs. It is the ultimate source of truth for how a logic circuit functions.
6. An **OR** gate outputs a `1` if _at least one_ input is a `1`. An **AND** gate outputs a `1` only if _all_ inputs are a `1`.
...
4. An overflow error occurs when the result of a calculation is a number greater than `15` and requires more than 4 bits to represent.
5. `1111`. (Start with `0001`, invert to `1110`, add 1 to get `1111`).
6. The **XOR** gate.
...
4. `-5` is `1011`. (Start with `0101`, invert to `1010`, add 1).
5. `1101` + `1011` = `11000`.
6. The final answer is `1000`, which is `8` in decimal.
...
4. Status flags allow the CPU to get many pieces of information (zero, negative, carry, overflow) from a single arithmetic operation (like subtraction), rather than needing separate, bulky hardware for every possible comparison.
5. It would calculate `$B - A$`. If the **Negative Flag** is `1`, it means the result was negative, which means that $A$ must have been greater than $B$.
6. A **NOR** gate.
...
To check if all four bits ($Y\_3, Y\_2, Y\_1, Y\_0$) are `1`, you would need a single **4-input AND gate**. Its output will only be `1` if all of its inputs are `1`.
...
4. A Multiplexer (or MUX) selects one of several data inputs and forwards it to a single output.
5. We would need **two** select lines. To represent four choices (0, 1, 2, 3), you need 2 bits (`` `00` ``, `` `01` ``, `` `10` ``, `` `11` ``).
6. $Y = (A \land \neg S) \lor (B \land S)$
...
**Diagram:** ![1-to-2 DEMUX CircuitVerse Diagram](images/demux-circuitverse.png) _Figure: A 1-to-2 DEMUX. The data input D is sent to two AND gates. The select line S (and its inverse) determines which of the AND gates opens to let the data through to its corresponding output._
...
4. It's simpler to have all units working at once and then select the desired output, rather than trying to build complex logic to turn the different units on and off.
5. The decoder takes the binary "opcode" from the select lines and turns it into a single "active" line to open the correct AND gatekeepers in the multiplexer.
6. The Z (Zero) flag will be `0` because the result is not `0000`. The N (Negative) flag will be `1` because the most significant bit is `1`.
...
4. A **combinational** circuit's output depends only on its current inputs. A **sequential** circuit's output depends on its current inputs _and_ its previous state (it has memory).
5. A feedback loop, where a gate's output is connected back to its input, is what allows a circuit to hold its state and "remember" a value even after the initial input is gone.
6. The "Write Enable" line acts as a gatekeeper. When it is ON, the latch is "open" and copies its data input. When it is OFF, the latch is "closed" and holds its current value, ignoring the data input.
...
4. The "16" represents the number of unique memory locations or addresses. The "4" represents the number of bits that can be stored at each of those locations.
5. The decoder takes the binary address from the Address Bus and activates a single "select line" to choose which of the many registers will be active for a read or write operation.
6. The `Write Enable` signal is necessary to differentiate between reading from and writing to a memory address. When it's OFF, the selected register outputs its data but doesn't change it. When it's ON, the selected register overwrites its current data with the data from the Data In bus.
...
4. You would need **256** individual 4-bit registers.
5. To represent 256 unique values ($2^8$), your Address Bus would need to be **8 bits** wide.
6. You would need an **8-to-256 decoder**.
...
4. **Fetch:** Get the instruction from memory. **Decode:** Determine what the instruction means. **Execute:** Activate the correct components to perform the instruction.
5. The **Program Counter (PC)** holds the address of the _next_ instruction to be fetched. The **Instruction Register (IR)** holds the _current_ instruction that is being decoded and executed.
6. The **Status Flags** (specifically, the Zero Flag in this case). The Control Unit's decision to jump is based on the state of this flag.
...
4. It solves the problem of converting a pure binary number (like `` `1101` ``) into a format where each decimal digit is represented by its own separate binary code (like `0001` and `0011`).
5. BCD is a system where each decimal digit (`0`-`9`) is encoded with its own dedicated 4-bit binary number.
6. The ROM-based approach is a "brute-force" lookup table. While large, its logic is extremely simple and repetitive, making it much easier to design, build, and debug in a block-based environment like Minecraft compared to a complex, multi-stage sequential circuit.

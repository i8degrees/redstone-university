...
That's it! Every single thing your computer does, from displaying this text, to playing a song, to running a complex game, is ultimately just a massive, coordinated manipulation of these simple `1`s and `0`s. Each individual `1` or `0` is called a **bit**, which is short for "binary digit."
...
*   The rightmost column is the `1`s place (`2⁰`).
*   The next column to the left is the `2`s place (`2¹`).
